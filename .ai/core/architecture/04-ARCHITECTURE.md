...
- Precision search (Phase 6.6, ADR-060): `search/precision/` orchestrator for browse search modes, multi-query RRF, alias/path filters, similar/by-path reads, optional rerank. Gated by `PRECISION_SEARCH_ENABLED=false`. Manifest: `supportsPrecisionSearch`.
...
**Default flow (flag OFF — unchanged):**
...
**Precision flow (Phase 6.6, `PRECISION_SEARCH_ENABLED=true`):**

```
PrecisionSearchOrchestrator (IPrecisionSearchService)
  → mode strategy (hybrid | semantic | fulltext | title)
  → multi-query RRF (optional)
  → optional IReranker
  → ISearchResultEnricher (snippet, links when extended)
  → paginate
```

Wiring: `create-precision-search-ports.ts` → `MemoryService.searchMemory`. Context retrieval (`Retriever`, `ContextService`) **unchanged**.
...
**Discovery:** REST `GET /api/v1/capabilities` and MCP `get_capabilities` share one builder path. Response includes `X-Protocol-Version` header. Manifest reflects extension flags (5.5 compression, 6.5 retrieval, **6.6 precision search**, 04.7 stewardship, 8.5 quality signals, 10.5 transport).
