...
Phase 19 exports raw metrics/traces/logs. Phase 25 needs a **semantic** telemetry model so the platform can learn how it is used GÇö retrieval, context, prompts, provider usage, sync GÇö without collecting user content.

Design: [.ai/phases/25-global-ai-intelligence/DESIGN.md](../../.ai/phases/25-global-ai-intelligence/DESIGN.md) -º3, -º8.
...
- Emitted at middleware boundary GÇö never inside `MemoryService`.
...
### Option A GÇö Reuse Phase 19 metrics only
- Pros: nothing new.
- Cons: no semantics; cannot drive analytics.

### Option B GÇö Semantic 10-event discriminated union + envelope (chosen)
- Pros: rich, typed, additive, OTLP-mappable, privacy-controlled.
- Cons: new types + redactor to maintain.

### Option C GÇö Log raw request/response bodies
- Pros: maximal signal.
...
Add types + recorder (noop sink) GåÆ OTLP adapter GåÆ Prometheus/Jaeger adapters GåÆ wire redactor. All additive, gated by master flag.

## Rollback

Noop sink / flag OFF GåÆ zero export, zero added latency target.
...
- [ADR-034 Observability Platform](../adr/034-observability-platform.md)
- [POLICY.md](POLICY.md)
