...
1. **No agent logic inside Ratary** � Planner, executor, and orchestrator belong to external systems.
2. **Ratary is storage-agnostic** � Any storage engine via ports.
3. **Ratary is retrieval-focused** � Bounded context within configurable budget.
4. **Ratary is context-focused** � Assembles relevant memories for agent consumption.
...
1. **No inbound agent state** � Ratary does not store agent execution state.
2. **No outbound agent commands** � Ratary does not invoke external systems.
3. **No bidirectional loops** � Ratary responds to requests; agents initiate.
...
Ratary tracks actors as metadata on memory operations. Actors are **metadata only** � no execution state is stored.
...
1. **No execution state** � Ratary stores who did what, not what they were doing.
...
Enterprise actors (Phase 10+) carry optional `organizationId` on `ActorMetadata`, aligned with `MemoryScope.organizationId` in `src/types/memory-scope.ts` and [ADR-002](../../adr/002-workspace-identity-model.md) / [ADR-010](../../adr/010-workspace-membership-rbac.md).
...
1. **Ratary is stateless** � No session state stored in the foundation.
...
4. **Audit-only** � Subscribers receive notifications; Ratary does not execute agent logic in handlers (external consumers only).
...
2. **No event handlers** � Ratary does not react to external events.
...
|-----|-------|-------------------|
| [ADR-001](../../adr/001-multi-source-retrieval.md) | Multi-source retrieval | Enables richer context |
| [ADR-002](../../adr/002-workspace-identity-model.md) | Workspace identity | Scope contract for Phase 9+ |
| [ADR-003](../../adr/003-embedding-storage-mvp.md) | Embedding storage | Vector retrieval foundation |
| [ADR-004](../../adr/004-repository-port-types.md) | Repository ports | Storage abstraction |
| [ADR-005](../../adr/005-content-object-store.md) | Content store | Future blob offload |
| [ADR-007](../../adr/007-multi-ai-workspace-scope.md) | Multi-AI workspace scope | Phase 9 `workspaceId` / `agentId` |
| [ADR-010](../../adr/010-workspace-membership-rbac.md) | Workspace membership RBAC | Phase 10 org scope + events |
