...
| ADR-013     | Security compliance              | Proposed     | SOC 2 framework — draft: [ADR-013-SECURITY-COMPLIANCE-DRAFT.md](./ADR-013-SECURITY-COMPLIANCE-DRAFT.md) · gap: [GAP-2026-07-29.md](../../evidence/adr-013-soc2/GAP-2026-07-29.md) |
...
| **ADR-070** | **Unified Code Memory Model**    | **Accepted** | Third graph plane for code structure (`CODE_NODE_KINDS` / `CODE_EDGE_TYPES`) · bridges to memories & entities via controlled stewardship only · separate `traverse_code` API · async deterministic indexer (Tree-sitter/SCIP; not `save_memory` hot path) · TS/JS then Python · `CODE_MEMORY_ENABLED=false` default · Phase **38** · Owner Accept C1–C7 2026-07-29 · **Phase 38 v1 implemented** (`code_*` schema · `ts-api-1.0` indexer · MCP/REST traverse · stewardship bridges; flag OFF) · source ARCH-0234 · full text private `.ai/core/adr/` |
