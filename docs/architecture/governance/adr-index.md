...
| **ADR-070** | **Unified Code Memory Model**    | **Accepted** | Third graph plane for code structure (`CODE_NODE_KINDS` / `CODE_EDGE_TYPES`) · bridges to memories & entities via controlled stewardship only · separate `traverse_code` API · async deterministic indexer (Tree-sitter/SCIP; not `save_memory` hot path) · TS/JS then Python · `CODE_MEMORY_ENABLED=false` default · Phase **38** · Owner Accept C1–C7 2026-07-29 · source ARCH-0234 · full text private `.ai/core/adr/` |
